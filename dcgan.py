...
class Trainer():
    def __init__(self, data, netD, netG, critD, critG, optimD, optimG):
        ...
        self.optimD = optimD
        self.optimG = optimG
        self.fixed_noise = torch.FloatTensor(BATCH_SIZE, nz, 1, 1).normal_(0, 1) # to monitor progress
    ...
    def run_epoch(self):
        ...
        for batch_ind, (real_imgs, _) in tqdm(enumerate(self.data)):
            ...
            for i in range(NUM_D_UPDATES):
                ...
                gen_imgs = self.netG(noise)
                d_out_real,real_embeddings = self.netD(real_imgs)
                d_out_fake,fake_embeddings = self.netD(gen_imgs)
                d_loss = self.critD(d_out_real, d_out_fake)
                d_loss.backward()
                self.optimD.step()
            ...
            # generator update
            _, real_embeddings = self.netD(real_imgs) # for embedding loss
            self.netG.zero_grad()
            self.netD.zero_grad()
            noise = torch.FloatTensor(len(real_imgs), nz, 1, 1).normal_(0, 1)
            noise = Variable(noise)
            if USE_GPU:
                noise = noise.cuda()
            gen_imgs = self.netG(noise)
            d_out, fake_embeddings = self.netD(gen_imgs)
            # g_loss = self.critG(d_out)
            g_loss = self.critG(fake_embeddings, real_embeddings, d_out)
            g_loss.backward()
            self.optimG.step()
    ...
    def save_fixed_noise_images(self, epoch):
        ...
        fake = self.netG(Variable(self.fixed_noise))
